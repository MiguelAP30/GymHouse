...
@type_quantity_router.get('/',response_model=List[TypeQuantity],description="Devuelve todos los tipos de cantidad de una comida")
def get_types_quantities()-> List[TypeQuantity]:
    db= SessionLocal()
    result = TypeQuantityRepository(db).get_all_type_quantities()
    return JSONResponse(content=jsonable_encoder(result), status_code=status.HTTP_200_OK)

@type_quantity_router.get('/{id}',response_model=TypeQuantity,description="Devuelve la información de un solo tipo de cantidad de una comida")
def get_type_quantity(id: int = Path(ge=1)) -> TypeQuantity:
    ...
    if not element:        
        return JSONResponse(
            content={            
                "message": "The requested type quantity was not found",            
                "data": None        
                }, 
...
@type_quantity_router.post('/',response_model=dict,description="Crea un nuevo tipo de cantidad de una comida")
def create_type_quantity(type_quantity: TypeQuantity = Body()) -> dict:
    ...
    return JSONResponse(
        content={        
        "message": "The type quantity was successfully created",        
        "data": jsonable_encoder(new_type_quantity)    
        }, 
        status_code=status.HTTP_201_CREATED
    )

@type_quantity_router.delete('/{id}',response_model=dict,description="Elimina un tipo de cantidad específico")
...
@type_quantity_router.put('/{id}',response_model=dict,description="Actualiza un tipo de cantidad específico")
